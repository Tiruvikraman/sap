...
import numpy as np
from process_data import exchange_table,generate_map,generate_graph,get_currency_conversion,vendor_details
...
def get_recommendation():
    global recommentaion,minimum_country_value,country_index,minimum_country
...
def generate_bot_response(message):
    answer=response(message+"The price of "+ result_sentence,10)
    return answer
...
def price_bar_graph():
    buf=generate_graph(prices)
    
    return send_file(buf, mimetype='image/png')

@app.route('/price-map')
def price_map():
    return generate_map(country_price_inr)
